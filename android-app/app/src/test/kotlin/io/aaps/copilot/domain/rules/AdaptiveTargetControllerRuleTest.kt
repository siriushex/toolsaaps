    ...
    fun returnsNoMatch_insideDeadband() {
        ...
        val decision = rule.evaluate(
            context(
                ...
                forecasts = listOf(
                    Forecast(now + 5 * 60_000, 5, 5.55, 5.2, 5.9, "test"),
                    Forecast(now + 30 * 60_000, 30, 5.52, 5.1, 5.9, "test"),
                    Forecast(now + 60 * 60_000, 60, 5.51, 5.0, 6.0, "test")
                )
            )
        )

        assertThat(decision.state).isEqualTo(RuleState.NO_MATCH)
        assertThat(decision.reasons).contains("inside_deadband")
    }

    @Test
    fun appliesTelemetryBias_whenIobHighAndFalling() {
        val now = System.currentTimeMillis()
        val decision = rule.evaluate(
            context(
                now = now,
        ...
        assertThat(decision.state).isEqualTo(RuleState.TRIGGERED)
        assertThat(decision.reasons.any { it.startsWith("telemetryBias=") }).isTrue()
    }
